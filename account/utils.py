...
def send_email_view(request, email):
    ...
    html_content = render_to_string('reset_password_email.html', {'reset_link': reset_link})

    # Plain text version of the email content
    text_content = f"Click on the following link to reset your password: {reset_link}"

    # Create EmailMultiAlternatives object and attach the HTML and plain text content
    msg = EmailMultiAlternatives(subject, text_content, 'testingbot202424@gmail.com', [email])
    msg.attach_alternative(html_content, "text/html")
